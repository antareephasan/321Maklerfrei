...
export const Review = ({
  ...
  const handleSubmit = async () => {
    ...
    // let afterDate = Date.now() - oldDate;
    // let nbr_images = imgMultiStepForm.selectedType.length;
    // let nbr_plans = planMultiStepForm.selectedType.length;
    // let total = nbr_images + nbr_plans;
    // let timeStamp = afterDate / total;
    // for (let i = 0; i < total; i++) {
    //   setTimeout(() => {
    //     if(i === total - 1 ) return openSnackbar(t(`List Is Created`), 'success', 3000);
    //     if(!enabled) openSnackbar(t(`Saving please wait... ${i+1}/${total}`), 'success');
    //   }, i*timeStamp);
    // }
    ...
    axios
      ...
      .then((response) => {
        ...
        //   "sepaClientSecret",
        //   response.data.sepaClientSecret
        // );
